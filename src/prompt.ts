...
import { getActionIcons } from './helpers/icons';
...
async function runOrReviseFlow(
  ...
  const icons = await getActionIcons();
  ...
  if (emptyScript) {
    ...
    console.log(
      dim(
        i18n.t(
          'This prompt did not produce a shell command. For conversation, run `ai chat`.'
        )
      )
    );
  }
  const answer: symbol | (() => any) = await p.select({
    ...
    options: [
      ...
      {
        label: icons.revise + i18n.t('Revise'),
      ...
      {
        label: icons.cancel + i18n.t('Cancel'),
